...
<!--      </div>-->
      ...
      <div class="stat-card info">
        <div class="stat-number">{{ totalPersonnesInteresseesRestauration }}</div>
        <div class="stat-label">Restauration</div>
        <div class="stat-detail">{{ pourcentagePersonnesInteresseesRestauration }}% intéressé(s)</div>
      </div>
    </div>
...
export default {
  ...
  setup() {
    ...
    // Calcul du total des personnes intéressées par la restauration (champ fast_food_check)
    const totalPersonnesInteresseesRestauration = computed(() => {
      return agentsTries.value.reduce((sum, agent) => {
        if (agent.fast_food_check == 1 || agent.fast_food_check === true) {
          // On additionne l'agent + ses proches
          return sum + 1 + parseInt(agent.nombre_proches || 0)
        }
        return sum
      }, 0)
    })

    // Calcul du pourcentage de personnes intéressées par la restauration
    const pourcentagePersonnesInteresseesRestauration = computed(() => {
      if (!totalPersonnes.value || totalPersonnes.value === 0) return 0
      return ((totalPersonnesInteresseesRestauration.value / totalPersonnes.value) * 100).toFixed(1)
    })
